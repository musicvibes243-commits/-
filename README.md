...
| Файл | Что внутри |
| --- | --- |
| `index.html` | русская версия: шапка, герой, о студии, практики, праздники, прайс, часы, отзывы, FAQ, как нас найти, контакты |
| `en/index.html` | английская версия той же страницы |
...
## Две демо-версии дизайна

| Папка | Стиль |
| --- | --- |
| `index.html`, `en/` | светлая: мох и золото, Lora + Raleway, фотографии в цвете |
| `editorial/` | тёмная редакционная: Playfair Display + IBM Plex Mono, чёрно-белые фото, бегущая строка |

Данные в обеих одинаковые — адрес, телефон, прайс, часы, отзывы. `editorial/`
использует те же фотографии из `assets/img/`, но со своими `styles.css`
и `main.js` и без переключателя тем: это осознанно одна визуальная тема.

## Две языковые версии

Русская — `index.html`, английская — `en/index.html`. Обе используют одни и те же
`assets/css/styles.css` и `assets/js/main.js`; в английской пути к ассетам ведут
на `../assets/`. Переключатель RU/EN стоит в шапке, версии связаны тегами
`<link rel="alternate" hreflang>`.

Тексты ошибок формы выбираются в `main.js` по `document.documentElement.lang`:
словарь `STRINGS` с ключами `ru` и `en`. Новый язык — добавить туда ключ,
скопировать страницу и поправить `lang` у `<html>`.
...
Собирает страницы в самодостаточные HTML: фотографии и шрифты вшиты внутрь,
снаружи не подгружается ничего. Такой файл открывается двойным щелчком без
интернета — им удобно показывать демо, когда хостинга ещё нет.

## QR-коды

```bash
pip install segno
python3 tools/make-qr.py "https://адрес-сайта" light   # или dark
```

Кладёт в `qr/`: SVG для печати, PNG для мессенджеров и HTML-карточку
формата A6 с названием, кодом и телефоном — её достаточно открыть
в браузере и распечатать или снять скриншотом.

При смене адреса сайта коды нужно перегенерировать: старые перестанут
вести куда нужно.
...
- Раздел «Праздники» (`#events`) — прежде всего проведение дней рождения,
  а также юбилеи, детские праздники, выпускные, девичники и корпоративы;
