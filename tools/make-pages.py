...
PAGES = [
{
 ...
 "video": ("otzyv-kotelnaya.mp4", "otzyv-kotelnaya-poster.jpg",
           "Котельная под ключ: показываем объект и слушаем хозяина",
           "Котёл, фильтр на вводе, расширительный бак, коллекторы тёплого пола "
           "и радиаторов — а в конце хозяин дома говорит, что получилось. "
           "Снято на сдаче объекта, выложено с его разрешения. Со звуком."),
 "photos": [
   ("kotelnaya-obshchiy-01.jpg",
    "Котельная в отделанном доме: электрокотёл с работающим дисплеем, бойлер, расширительный бак, "
    "два коллекторных шкафа и насос на стене из вагонки",
    "Котельная целиком", "Собрано и запущено — так это выглядит в готовом доме"),
   ("kollektor-rashodomery-01.jpg",
    "Коллектор тёплого пола на шесть контуров с расходомерами, на каждой трубе маркером подписана комната",
    "Коллектор тёплого пола", "Контуры подписаны по комнатам — потом не гадать, что перекрывать"),
   ("tepliy-pol-kontury-01.jpg",
    "Контур водяного тёплого пола уложен улиткой на армирующей сетке, по периметру демпферная лента",
    "Контуры тёплого пола", "Кладём улиткой: подача и обратка идут рядом, пол греет ровно"),
   ("konvektor-v-polu-01.jpg",
    "Открытый внутрипольный конвектор у панорамного окна: теплообменник, гибкая подводка и строительный уровень сверху",
    "Конвектор в полу", "Ставим по уровню: у панорамного окна радиатор перекрыл бы вид"),
   ("radiator-montazh-02.jpg",
    "Радиатор под окном с подводкой снизу, трубы в красной и синей гофре уходят в пол",
    "Радиатор с подводкой из пола", "Трубы идут в гофре: при необходимости меняются без вскрытия стен"),
   ("razvodka-karkas-01.jpg",
    "Трубы воды и канализации разведены по стойкам каркаса, на полу сетка под стяжку тёплого пола",
    "Разводка по каркасу", "Вода и канализация разведены до зашивки стен — потом уже не переделать"),
   ("boiler-ariston-01.jpg",
    "Настенный водонагреватель Ariston с обвязкой, фильтром и запорными кранами",
    "Водонагреватель с обвязкой", "Фильтр на входе и краны на каждой линии — обслуживать можно не сливая воду"),
   ("razvodka-sanuzel-01.jpg",
    "Санузел с разведёнными по стене трубами воды и канализации, установлен унитаз",
    "Разводка в санузле", "Выводы под раковину и унитаз, канализация с ревизией — всё собрано до отделки"),
 ],
...
},
{
 ...
 "video": ("elektrika-razvodka.mp4", "elektrika-razvodka-poster.jpg",
           "Разводка по дому до отделки",
           "Проходим по объекту: кабель к каждой розетке и выключателю, "
           "подрозетники выставлены, гофра по стенам, рядом трубы воды. "
           "Снято до стяжки — дальше всё это уйдёт под пол и штукатурку."),
 "photos": [
   ("shchit-avtomaty-01.jpg",
    "Собранный электрощит в деревянном доме: два ряда автоматов, нулевая и заземляющая шины",
    "Щит в готовом доме", "Отдельный автомат на каждую группу, ноль и земля разведены по шинам"),
   ("shchit-uzo-01.jpg",
    "Электрощит с вводным автоматом, УЗО и автоматами на группы, разводка по шинам",
    "Защита на каждую группу", "Вводной автомат, УЗО и свой автомат на линию — выбьет только проблемную"),
   ("razvodka-komnata-01.jpg",
    "Комната до отделки: подрозетники с выведенными кабелями, штробы, гофра по полу, трубы воды и канализации",
    "Всё разведено до стяжки", "Подрозетники, штробы и гофра под кабель — заодно вода и канализация"),
 ],
...
},
{
 ...
 "photos": [
   ("avtopoliv-uchastok-01.jpg",
    "Большой газон перед домом, по всей площади одновременно работают дождеватели",
    "Участок целиком", "Газон разбит на зоны и поливается по очереди — напора хватает каждой"),
   ("avtopoliv-forsunka-01.jpg",
    "Выдвижной дождеватель Hunter с красным соплом, утопленный вровень с газоном",
    "Форсунка в газоне", "В покое утоплена вровень с землёй — не мешает косилке"),
   ("avtopoliv-nasosnaya-01.jpg",
    "Насосный узел автополива: накопительная ёмкость, насос, гребёнка зон и контроллер на стене",
    "Насосная станция полива", "Ёмкость, насос и гребёнка зон в одном узле — каждая линия перекрывается отдельно"),
   ("avtopoliv-teplica-01.jpg",
    "Труба полива проложена вдоль цоколя теплицы и заведена внутрь, за стеклом грядки с томатами",
    "Своя ветка на теплицу", "Труба заведена под грядки — теплица поливается отдельно от газона"),
   ("avtopoliv-pult-01.jpg",
    "Контроллер полива Hunter в запираемом металлическом боксе, рядом счётчик воды",
    "Пульт в закрытом боксе", "Расписание задаётся один раз — дальше полив идёт ночью, без вас"),
   ("avtopoliv-klapany-boks-01.jpg",
    "Электромагнитные клапаны автополива с проводами в лючке, засыпанном щебнем",
    "Клапаны в лючке", "Стоят на щебне: вода уходит, коробка не превращается в лужу"),
 ],
 "other": [("Отопление и тёплые полы", "otoplenie.html"),
...
def video_html(p):
    """Ролик с объекта. Если по направлению ролика нет — секции нет вовсе.
    Грузится только по нажатию (preload="none"): ролик весит около мегабайта,
    а половина посетителей приходит с мобильного интернета."""
    v = p.get("video")
    if not v:
        return ""
    src, poster, title, lead = v
    return f'''
<!-- ================= ВИДЕО С ОБЪЕКТА ================= -->
<section class="clip-sec">
  <div class="wrap">
    <div class="head">
      <p class="eyebrow">Видео с объекта</p>
      <h2>{title}</h2>
      <p class="lead">{lead}</p>
    </div>
    <video class="objvideo" src="assets/video/{src}" poster="assets/video/{poster}"
...
def photos_html(p):
    """Блок с фотографиями. Если снимков по направлению нет — секции нет вовсе:
    пустая галерея выглядит хуже, чем её отсутствие."""
    if not p.get("photos"):
        return ""
    shots = []
    for fname, alt, cap, sub in p["photos"]:
        shots.append(f'''      <li class="shot">
        <a href="assets/photos/{fname}" target="_blank" rel="noopener">
          <img src="assets/photos/{fname}" alt="{alt}" loading="lazy" decoding="async">
          <span class="shot__cap"><b>{cap}</b><i>{sub}</i></span>
        </a>
      </li>''')
    return f'''
<!-- ================= РАБОТЫ ================= -->
<section id="raboty" class="works">
  <div class="wrap">
    <div class="head">
      <p class="eyebrow">Наши работы</p>
      <h2>{"Снимок с объекта" if len(p["photos"]) == 1 else "Снимки с объектов"}</h2>
    </div>
    <ul class="gallery{" gallery--one" if len(p["photos"]) == 1 else " gallery--three" if len(p["photos"]) == 3 else ""}">
{chr(10).join(shots)}
    </ul>
  </div>
</section>
